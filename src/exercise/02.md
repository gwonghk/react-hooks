...
1. Easily move all hooks logic (useState, useEffect) into a custom hook, useLocalStorageState. Just make sure to return the same outputs, and take in the same inputs, and also follow convention of useFunc
2. LazyStateInitialization - if given a func, useState will only call it when it needs the value. If the value is expensive to calculate, put it in a () => value and useState will not spam it on every render
3. When dealing with localStorage
   1. serialize / deserialize
   2. allow defaultValue to be set via expensive function, so it doesnt have to
   3. handle key change
4. useRef gives you an obj that you can mutate without triggering rerenders
5. command . opens dropdown to autofix lint issues!
6. React Hooks Lifecycle
   1. LazyInit is only called once on mount
   2. Parent component is rendered start and end, before child component is invoked and render start
   3. Effect callbacks run in order:
      1. useEffect(() => {})
      2. useEffect(() => {}, [])
      3. useEffect(() => {}, \[var\])
   4. on Unmount, all cleanups run
   5.
