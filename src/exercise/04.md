...
1. types of state
   1. managed state = explicitly managed
   2. derived state = based on other state
2. Array() constructor
   1. Array(9) creates a .length 9 array
   2. Array(9).fill(null) creates [ nullx9 ]
3. Remember to put the initial load in the lazyInitState
4. add console.log('%c text', 'color: LightCoral/MediumSpringGreen') for colorful console logs!
5. OnClick needs to call/reference a function, not execute it directly on render!

## Background

A `name` is one thing, but a real UI is a bit different. Often you need morew
