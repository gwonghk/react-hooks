...
1. Async functions automatically return a promise, so you cant just directly use async/await in useEffect, since it will treat the returned promise as cleanup function
2. Instead, inside useEffect, declare an Async function, and call it
3. Might be easier to declare the Async function outside, then call .then() on the promise instead of using async/await
4. A key prop can be provided to trigger re-renders, to reset a component
5. ErrorBoundary takes a FallbackComponent so we can just display error msg component, and maybe add a reset button
6. ErrorBoundary also takes a resetKeys array, so that if those keys change the component auto resets
7.
